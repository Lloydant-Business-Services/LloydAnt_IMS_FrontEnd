...
export default class NewLeaveRequest extends React.Component {
  state = {
  ...
  getLeaveRequestHistoryByRole = () => {
    fetchData(`/LeaveRequestManagement/GetLeaveRecordsHistory?departmentId=${this.state.staffPayLoad?.departmentId ? this.state.staffPayLoad?.departmentId : 0}&roleId=${this.state.payLoad?.roleId}`, (data) => {
